...
An \texttt{AnalysisComponent} using the \texttt{HybridCache} as input has access to all models. In order to make the implementation of an incremental analysis easier, \texttt{HybridCache} also allows to directly obtain the parts of the codemodel that did change.

When adapting a non-incremental \texttt{AnalysisComponent}-class that directly works with code-, build- or variabilitymodel, the non-incremental component expects those models to be passed to it through three distinct \texttt{AnalysisComponent}s. Each of those three components are passed to the non-incremental component through the constructor of the class. 
 \texttt{HybridCache} itself is a single object and not an implementation of \texttt{AnalysisComponent}. Furthermore the models represented through the \texttt{HybridCache} be directly accessed through method calls. In order to allow for reuse of existing analysis-implementations like the \texttt{UnDeadAnalyzer}, an adapter class was created that transforms the models stored in \texttt{HybridCache} to three separate OutputComponents.
